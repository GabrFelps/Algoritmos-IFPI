...
function main(){
    ...
    while (distancia_percorrida < 600 && litros_usados < 50){
        let distancia = get_number('Quantos kms o carro andou? ')
        distancia_percorrida += distancia
        let litros = get_number('Quantos litros o carro utilizou? ')
        ...
        if (distancia_percorrida < 600 && litros_usados < 50){
            print(`Parada ${qtd_paradas} -> O carro parou antes do destino, mas não foi por falta de combustível (Faltam ${(50-litros_usados).toFixed(1)}L)`)
            print(`O carro ja andou ${distancia_percorrida}km, faltam ${(600-distancia_percorrida)}km.`)
            print(`Consumo da parada anterior até essa parada: ${consumo}`)
        }else if (distancia_percorrida<600 && litros_usados >= 50){
            print(`Parada ${qtd_paradas} -> O carro parou antes do destino pois a gasolina acabou (Faltavam ${600 - distancia_percorrida}km)`)
            print(`Consumo da parada anterior até essa parada: ${consumo.toFixed(1)} km/l`)
        }
    }
